...
(function ($) {
   ...
   $(".product-slider").owlCarousel({
	...
	proQty.on('click', '.qtybtn', function () {
        ...
        let rowId = $(this).parent('.pro-qty').find('input').data('rowid');
...
function addCart(productId) {
    $.ajax({
        ...
        success: function(response) {
            ...
            $('.select-total').text('$' + response['total']);
            var cartHover_tbody = $('.cart-hover tbody');
            var cartHover_existItem = cartHover_tbody.find('tr[data-rowId="'+response['cart'].rowId+'"]');

            if(cartHover_existItem.length) {
                $('.product-selected p').text('$' + response['cart'].price.toFixed(2) + 'x' + response['cart'].qty);
            }else {
                var newItem = 
                '<tr data-rowId='+response['cart'].rowId+'>\n' +
                '<td class="si-pic"><img style="height:100px" src="front/img/products/'+ response['cart'].options.images[0].path +'" alt=""></td>\n' +
                '<td class="si-text">\n' +
            ...
            alert(`Add success Product ${response['cart'].name}`);
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if(qty === 0){
                ...
                cartHover_existItem.find('.product-selected p').text('$'+response['cart'].price.toFixed(2) + ' x ' + qty);
            }
